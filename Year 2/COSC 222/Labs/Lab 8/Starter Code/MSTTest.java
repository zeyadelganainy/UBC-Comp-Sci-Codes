...
//		graph = randomGraph(6);	// random graph with the given number of vertices (see disclaimer in randomGraph())
	...
	private static MSTGraph randomGraph(int numVertices) {
		...
		/* 
		 * DISCLAIMER:
		 * Note that this method may occasionally generate disconnected 
		 * graphs. For example, in a 6-node graph, you may have nodes 0, 1, 
		 * and 2 connected to one another, and nodes 4, 5, and 6 also connected
		 * to one another, but no connection between the two groups of 3 nodes.
		 * This will probably cause an infinite loop, depending on how you write 
		 * the MSTGraph.getMST() method. You are not required to account for the 
		 * chance of this occurring.  If you do get an infinite loop, you should 
		 * manually stop your code, then ensure that the graph is actually
		 * disconnected by checking the adjacency list and drawing the generated 
		 * graph. 
		 * 
		 */
		...
		for(int v = 0; v < numVertices; v++) {
			...
			//adding the one guarantees every vertex has at least one edge, i.e. no node is disconnected
