...
public class BackupRestore {
	...
	public static int backup(String filename, double partSize) {
		...
		try (DataInputStream dis = new DataInputStream(new FileInputStream(filename))) {
			for (int i = 1; i <= parts; i++) {
				int count = 0;
				dos = new DataOutputStream(new FileOutputStream(filename + "." + i + ".pdf"));
				while (dis.available() > 0 && count <= partSize * 1000000) {
					dos.write(dis.read());
					count++;
				}
				dos.close();
	...
	public static int restore(String filename, int numberOfPieces) {
		File f = new File(filename);
		try (DataOutputStream dos = new DataOutputStream(new FileOutputStream(filename+"1.pdf"))) {
		for (int i = 1; i <= numberOfPieces; i++) {
				DataInputStream dis = new DataInputStream(new FileInputStream(filename + "." + i + ".pdf"));
